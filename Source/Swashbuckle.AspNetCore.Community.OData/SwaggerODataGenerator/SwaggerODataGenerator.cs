...
namespace Swashbuckle.AspNetCore.Community.OData.DependencyInjection
{
        ...
        : ISwaggerProvider
    {
        ...
        private readonly SwaggerODataGeneratorOptions options =
            options.Value ?? new SwaggerODataGeneratorOptions();
        ...
        )
        {
            if (!this.options.SwaggerDocs.TryGetValue(documentName, out var info))
            {
                throw new UnknownSwaggerDocument(
                    documentName,
                    this.options.SwaggerDocs.Select(d => d.Key)
                );
            }
            ...
            if (!this.options.EdmModels.TryGetValue(info.OdataRoute, out var edmModel))
            {
                throw new UnknownODataEdm(
                    info.OdataRoute,
                    this.options.EdmModels.Select(d => d.Key)
                );
            }
        ...
        private sealed class UnknownODataEdm(
            string documentName,
            IEnumerable<string> knownEdmModels
        )
